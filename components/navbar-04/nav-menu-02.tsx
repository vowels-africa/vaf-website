...
export const NavMenu = ({ onItemClick, ...props }: NavMenuProps) => {
  ...
  useEffect(() => {
    const sections = ["framework", "solutions", "pricing", "faq"];
  ...
  <NavigationMenu {...props}>
    <NavigationMenuList className="gap-4 space-x-0 data-[orientation=vertical]:flex-col data-[orientation=vertical]:items-start">
      ...
      <NavigationMenuItem>
        <NavigationMenuLink asChild>
          <Link href="#faq" className={cn("border border-transparent focus:bg-transparent focus:text-white active:bg-transparent hover:bg-transparent hover:text-white !rounded-full px-4", activeSection === "faq" && "!bg-[#69e3e3] !focus:bg-[#69e3e3] active:bg-[#69e3e3] !hover:bg-[#69e3e3] !text-[#010c23]")} onClick={() => handleNavClick("faq")}>FAQ</Link>
        </NavigationMenuLink>
      </NavigationMenuItem>
    </NavigationMenuList>
  </NavigationMenu>
